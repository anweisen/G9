...
abstract interface class HurdleType {
  String get paragraph;
  String get desc;
}

class HurdleCheckResult {
  final HurdleType hurdle;
  final String text;

  HurdleCheckResult(this.hurdle, this.text);
}

enum AdmissionHurdle implements HurdleType {
  ...
  static display(Choice? choice) => [min48dml, min100abi5, min200in40, min5je32, min1je, no0, no0sem];
  ...
  static List<HurdleCheckResult> check(Choice choice, Map<Subject, Map<Semester, SemesterResult>> result, ResultsFlags flags, GradesDataProvider provider) {
    ...
    for (Subject subject in choice.subjects) {
      for (Semester semester in Semester.qPhaseSeminar) {
        ...
        if (sr.used && sr.grade < 1) {
          checkResults.add(HurdleCheckResult(AdmissionHurdle.min1je, "${subject.name} in ${semester.display}"));
        }
    ...
    if (pointsIn40 < 200) {
      checkResults.add(HurdleCheckResult(AdmissionHurdle.min200in40, "$pointsIn40 von 200"));
    }
    ...
    if (pointsInAbi5 < 100) {
      checkResults.add(HurdleCheckResult(AdmissionHurdle.min100abi5, "$pointsInAbi5 von 100"));
    }
    ...
    if (pointsInDML < 48) {
      checkResults.add(HurdleCheckResult(AdmissionHurdle.min48dml, "$pointsInDML von 48"));
    }
    ...
    for (var entry in seminarGrades) {
      if (entry.grade == 0) {
        checkResults.add(HurdleCheckResult(AdmissionHurdle.no0sem, entry.type.name));
      }
    }
...
enum GraduationHurdle implements HurdleType {
  ...
  static List<HurdleCheckResult> check(Choice choice, Map<Subject, Map<Semester, SemesterResult>> result, ResultsFlags flags, GradesDataProvider provider) {
    ...
    for (Subject subject in choice.abiSubjects) {
      ...
      if (sr.grade < 4) {
        checkResults.add(HurdleCheckResult(GraduationHurdle.min1je, "In ${subject.name}: ${sr.grade} von 4"));
      }
    ...
    if (min5Subjects.length < 3) {
      checkResults.add(HurdleCheckResult(GraduationHurdle.min5je3, "${min5Subjects.length} von 3"));
    }
    if (!min5Subjects.contains(Subject.deutsch) && !min5Subjects.contains(Subject.mathe) && !min5Subjects.contains(choice.lk)) {
      checkResults.add(HurdleCheckResult(GraduationHurdle.min5je3, "Weder Deutsch, Mathe oder ${choice.lk.name} darunter"));
    }
    ...
    for (var entry in taskFieldMap.entries) {
      ...
      if (under4.length > 1) {
        checkResults.add(HurdleCheckResult(GraduationHurdle.max1u4fields, "${under4.length}x ${entry.key.display}: ${under4.map((e) => "${e.$1.name} (${e.$2.grade} von 16)").join(", ")}"));
      }
    }
    ...
    if (validSgNtg.isNotEmpty) dmnsSubjects.add(validSgNtg.first); // eigentlich immer mindestens eins vorhanden (außer keine Einträge)
    ...
    if (dmnsUnder16.length > 1) {
      checkResults.add(HurdleCheckResult(
        ...
        "${dmnsUnder16.length}x: ${dmnsUnder16.map((e) => "${e.name} (${result[e]![Semester.abi]!.grade} von 16)").join(", ")}"
      ));
    }

    // min100: In Summe min. 100 Punkte in den Abiturprüfungen (4-fache Wertung)
    if (flags.pointsAbi < 100) {
      checkResults.add(HurdleCheckResult(GraduationHurdle.min100, "${flags.pointsAbi} von 100"));
    }

    // min300: Insgesamt min. 300 Punkte
    if (flags.pointsTotal < 300) {
      checkResults.add(HurdleCheckResult(GraduationHurdle.min300, "${flags.pointsTotal} von 300"));
    }
